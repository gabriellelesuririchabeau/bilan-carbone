...
async function loadTeacherProfileName(userId: string) {
  ...
  async function handleSaveTeacherEdit() {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("update_teacher_profile", {
        body: {
      ...
      if (error) {
        setMessage(`Erreur modification compte : ${error.message}`);
        return;
      }

      if (data?.error) {
        setMessage(`Erreur modification compte : ${data.error}`);
        return;
      }
                ...
                {editingTeacherId ? (
                  <div ref={teacherEditFormRef} style={{ ...styles.innerCardFull, scrollMarginTop: 24 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
                      ...
                      <input style={styles.input} placeholder="Email du compte" value={editingTeacherEmail} onChange={(e) => setEditingTeacherEmail(e.target.value)} />
                    </div>
      ...
      <aside style={styles.sidebar} className="teacher-sidebar-organized">
        ...
        {selectedSessionId ? (
          <>
            ...
            <details className="teacher-sidebar-section" open={teacherMenu === "sessions" || (screen as string) === "teacher_session_settings"} style={styles.sidebarSection}>
              <summary style={getSessionSectionTitleStyle()}><span className="teacher-sidebar-chevron" style={styles.sidebarChevron}>›</span><span style={styles.sidebarSectionIcon}>⚙️</span><span>{lang === "en" ? "SESSION" : "SESSION"}</span></summary>
              <button
